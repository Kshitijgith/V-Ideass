...
export default function LoginPage() {
  ...
  const [isPageLoading, setIsPageLoading] = useState(true); // State for global page loading
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      setIsPageLoading(false); // Set page loading to false after a delay
    }, 1000);
  ...
  return (
    <div className="h-80p w-30p sm:w-90p sm:h-70p border rounded-xl flex flex-col justify-center items-center">
      <div className="bg-gray-100 border rounded-lg shadow-xl w-100p h-100p flex flex-col">
        ...
        >
          {isLoading && (
            <div className="sm:h-100p sm:w-100p h-30p bg-gray-100 opacity-50 absolute z-10 flex items-center justify-center">
              <div className="w-12 h-12 border-4 border-blue-500 border-t-transparent rounded-full animate-spin"></div>
            </div>
          )}
          ...
          <div className="h-40p w-100p flex flex-col items-center justify-center">
